...
def load_data(
    ...
    try:
        ...
        logger.info(f"Location columns: {list(locations.columns)}")
        logger.info(f"Population columns: {list(people_per_block.columns)}")
        ...
        # Create pop2022 column from units if it doesn't exist
        if "pop2022" not in people_per_block.columns:
            if "units" in people_per_block.columns:
                logger.info("Using 'units' column as population data ('pop2022')")
                # Aggregate units by block to get total population per block
                people_per_block = people_per_block.groupby('block')['units'].sum().reset_index()
                people_per_block.rename(columns={'units': 'pop2022'}, inplace=True)
            else:
                raise ValueError("Population file missing required column: 'units' or 'pop2022'")
...
def main():
    ...
    try:
        # File paths
        locations_file = "data/test dataset state 56 block_id location_id unit_count round one.dta"
        population_file = "data/test dataset state 56 block_id location_id unit_count round one.dta"
